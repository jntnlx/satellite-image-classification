...
def get_resnet_model(
    ...
    try:
        ...
        print(f"Success Using Model: Loaded '{model_variant}' model variant and corresponding weights (ImageNet Pre-Training).\n")

    except Exception as e:

        raise RuntimeError(f"Error: Invalid model ('{model_variant}') construction: {e}")
    ...
    if use_architectural_mod == True:
        ...
        if stem_conv == "random":
            ...
            pretrained_weights = model.conv1.weight.detach().clone()  # Shape: [64, 3, 7, 7]
